...
// Modified by Daniel Alex Finkelstein / Polytechnic Institute of NYU / 2010
...
inline void reset(line_buffer & lb) {
	lb.set_buffer(0);
}
void add(line_buffer & lb, char c) {
	lb.replace(lb.textlen(), &c, 1, 0);
}
...
void process(istream & in, string_match & target) {
	line_buffer para;
	int was_eol = 1;
	target.reset();
	...
	while (!in.eof()) { // looping over chars in file
		...
		if (eolgetc(inchar, in)) { // end of line?
			...
			if (was_eol)
				reset(para); // two newlines, para break
			else {
		...
		if (in.eof())
			return; // maybe eolgetc() above hit eof?
		was_eol = 0; // if we got here, have non-line-break char

		if (!target.match(casify(inchar)))
			add(para, inchar);
		else {
			...
			while (!in.eof()) { // then previously unread part
				if (eolgetc(inchar, in) && !rest) {
					...
					if (eolgetc(inchar, in))
						break; // and test again
				}
